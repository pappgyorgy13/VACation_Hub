...
interface BanCardProps {
  ban: Ban
  onVote?: (banId: string, vote: boolean) => void
  onComment?: (banId: string) => void
}
...
export function BanCard({ ban, onVote, onComment }: BanCardProps) {
  const [loading, setLoading] = useState(false)

  const handleVote = async (vote: boolean) => {
    setLoading(true)
    try {
      await onVote?.(ban.id, vote)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-gray-400">
            Steam ID: {ban.steam_id}
          </p>
          {ban.evidence && (
            <p className="mt-2 text-sm text-gray-300">{ban.evidence}</p>
          )}
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1 rounded-md bg-cs2-light px-2 py-1 text-sm text-green-400 hover:bg-green-400/20 disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1 rounded-md bg-cs2-light px-2 py-1 text-sm text-red-400 hover:bg-red-400/20 disabled:opacity-50"
          >
...
interface BanListProps {
  ...
  onCategoryChange: (category: BanCategory) => void
  onVote?: (banId: string, vote: boolean) => void
  onComment?: (banId: string) => void
}

export function BanList({ bans, category, onCategoryChange, onVote, onComment }: BanListProps) {
  return (
