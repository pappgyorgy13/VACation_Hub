...
DROP TABLE IF EXISTS public.ban_comments CASCADE;
...
CREATE POLICY "Authenticated users can insert bans" ON public.bans FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Users can update own bans" ON public.bans FOR UPDATE USING (auth.uid() = reporter_id);
...
CREATE POLICY "Anyone can view votes" ON public.ban_votes FOR SELECT USING (true);
CREATE POLICY "Authenticated users can insert votes" ON public.ban_votes FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Users can update own votes" ON public.ban_votes FOR UPDATE USING (auth.uid() = user_id);

-- Comments on bans
CREATE TABLE public.ban_comments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ban_id UUID REFERENCES public.bans(id) ON DELETE CASCADE,
  user_id UUID REFERENCES public.profiles(id),
...
CREATE POLICY "Anyone can view comments" ON public.ban_comments FOR SELECT USING (true);
CREATE POLICY "Authenticated users can insert comments" ON public.ban_comments FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Users can update own comments" ON public.ban_comments FOR UPDATE USING (auth.uid() = user_id);

-- Create indexes for better performance
CREATE INDEX idx_bans_steam_id ON public.bans(steam_id);
CREATE INDEX idx_bans_category ON public.bans(category);
CREATE INDEX idx_bans_created_at ON public.bans(created_at DESC);
CREATE INDEX idx_matches_steam_id ON public.matches(steam_id);
